...
#6 month timepoint, we have 2 datasets from the same mouse, GFP positive LSKs and GFP negative LSKs
exp1 <- Read10X(data.dir= "inputs/exp1/GFPpos/")
exp1.gfpneg <- Read10X(data.dir= "inputs/exp1/GFPneg/") 
...
#create seurat objects for each of the datasets
#exp1.sobj <- CreateSeuratObject(counts = exp1, min.cells = 200, 
#                                min.features = 500,project = "mo06_gfp")

#exp1.nogfp.sobj <- CreateSeuratObject(counts = exp1.gfpneg, min.cells = 200, 
#                                      min.features = 500,project = "mo06_nogfp")
...
# annotate the mitcohondrial percentage, remove cells with poor QC critera, 
# and remove pseudogenes and ribosomal genes
#exp1.sobj <- preProcessDataset(exp1.sobj,30000,4)
#exp1.nogfp.sobj <- preProcessDataset(exp1.nogfp.sobj,30000,4)
...
dataset.integrated <- ScaleData(dataset.integrated,assay = "RNA")
DoHeatmap(dataset.integrated, features = top10$gene,assay = "RNA") + NoLegend()


write.csv(x,"outputs/cluster_markers.csv")
...
write.csv(x,"outputs/HSPC_DEGS_across_ages.csv")
...
DoHeatmap(dataset.integrated, features = top10$gene,assay = "RNA",disp.min = -1.5, disp.max = 1.5) + NoLegend()  + scale_fill_gradientn(colors = c("white","grey50", "black"))
...
## Step 22. Identify surface markers for cluster 6 (MPP3/MPP5)

we perform gene expression as above. But here we filter on genes that are associated with receptor signalling and adhesion, these classes of molecules are on the cell surface and may have good antibodies already. Just bear in mind that in the GO terms there are probably a number of genes that are not on the cell surface. 

```{r surfacemarkers_cluster, message = FALSE, warning = FALSE} 
#### What markers could we use to purify the cells in the new cell state? ###
Idents(dataset.integrated) <- "cluster"

age.markers <- FindMarkers(dataset.integrated,ident.1 = c("7","8","9","10"),only.pos = TRUE,test.use = "LR",logfc.threshold = 0.2,assay = "RNA")
...
markers.filtered <- c(GO.sets['GO:0007166 cell surface receptor signaling pathway'][[1]],
                      GO.sets["GO:0007155 cell adhesion"][[1]])
putative.markers <- intersect(rownames(age.markers),markers.filtered)


dataset.integrated <- ScaleData(dataset.integrated, assay = "RNA")
DoHeatmap(dataset.integrated, features = putative.markers[1:20],assay = "RNA") + NoLegend()
...
## Step 23. Check Lineage Markers and Gene Expression Across Ages
...
tiff("/Users/jasoncosgrove/Desktop/psuedotime.tiff", width = 9, height = 6, units = 'in', res = 300)
ggplot(df, 
...
ggplot(df, 
       aes(x = pseudotime_dpt_ranked, 
           y = cluster, colour = marker)) + geom_quasirandom(groupOnX = FALSE) +
    scale_colour_gradientn(colors = c("grey10","green")) + theme_classic() +
    xlab("DPT") + ylab("Timepoint") +
    ggtitle("Cells ordered by pseudotime")
...
In this experiment we analysed LSKs from mice that were 6 (10X V2),12 (10X V3) and 18 months (10X V3) old. Data were integrated and underwent semi-supervised clustering and annotation. We then assessed clusters annd gene expression patterns that were upregulated at the different ages and modelled differentiation using a diffusion map based approach. 
