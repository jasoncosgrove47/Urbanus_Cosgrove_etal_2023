...
```{r, echo=FALSE}
d <- read.delim("~/Documents/Leila/NL lab/projet NL/barcode mouse NL/CAGCreERT2:RAG+ 2/%GFP.txt")
M <- d[1:98,1:4] #remove empty raws
ggplot(M, aes(x=tam, y=M)) + geom_boxplot() + ylab("GFP expression in myeloid cells") + xlab ("") + theme_bw() #plot data
...
ggplot(pp, aes(fill=class, x=type, y=contrib)) + geom_bar(position="fill", stat="identity") + theme_bw() + ylab("Contribution to cell type (%)") + xlab("") +scale_fill_discrete(name="Category")
...
ggplot(A, aes(, x=tot, y=Pgen, col=class)) + geom_point() + scale_y_log10()+ xlab("abundance in reads")+ theme_bw()
...
```{r, echo=FALSE}
#load data
dab<- read.delim("BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 renorm a vs b.txt")
dab <- dab[,-1]#remove barcode name column
dab <- cbind(dab, (dab$vara+dab$varb))#compute sum of replicates
dimnames(dab)[[2]][8]<- "sum"#name the sum column
#subset data
d<- dab[which(dab$month==15),]#select data at 15 months
d <- d[which(d$type=="M"),] #select M data
d <- d[which(d$organ!="SP"),] #remove spleen data
z <- d[which(d$mouse %in% c("X1","X2","X3","X4")),] # select experimental mice
z$mouse <- sub("X","",z$mouse)#substract X in the name of the mouse
#plot
ggplot(z[z$organ=="BM",], aes (x= sum*100, color=factor(mouse))) + stat_ecdf(geom = "step") + ylab("cumulative clone size") + xlab("clone size (decreasing order)")+ theme_bw() + labs(color = "mouse")+ theme_bw()
...
```{r, echo=FALSE}
#load data
D <- read.delim("~/Documents/Leila/NL lab/projet NL/barcode mouse NL/CAGCreERT2:RAG+ 2/%cell in GFP+ and neg over time.txt")
#format data
...
#ploting
ggplot(x, aes(x=month, y=numbM)) + geom_point(aes(color=mouse))+ theme_bw() + facet_wrap(~GFP, scales="free_y")+scale_y_log10()+ ylab("myeloid cell numbers")+ xlab("Month")+ geom_smooth(method = lm, se = TRUE)
...
```{r, echo=FALSE}
#plot cumulative clone size distribution
#prepare data
dab<- read.delim("BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 renorm norm cell numb a vs b.txt")
dab <- dab[,-1] #remove barcode name column
dab <- cbind(dab, (dab$vara+dab$varb)) # sum duplicates
dimnames(dab)[[2]][8]<- "sum" # rename column sum
#remove unnecessary samples
d<- dab[which(dab$type=="M"),] #select M samples
d <- d[which(d$month <13),] #select data up to 12 months
z <- d[which(d$mouse %in% c("X1","X2","X3","X4")),] # select experimental mice
z$mouse <- sub("X","",z$mouse) # remove X in the name of the mouse
#plot cum distrib
ggplot(z, aes (x= sum, color=factor(month))) + stat_ecdf(geom = "step") + ylab("cumulative clone size") + xlab("clone size (decreasing order)")+ theme_bw() + labs(color = "month")
...
ggplot(data = bcm1, aes(x = month, y = richness)) + theme_bw() + 
  scale_x_continuous(name = "Month") + scale_y_continuous(name = "Richness") +
  geom_jitter(mapping = aes(colour = Mouse), width = .1) +
  geom_line(data = bcm.pred.pw, mapping = aes(x = month, y = richness)) +
  geom_ribbon(data = bcm.pred.pw, mapping = aes(ymin = richness.l, ymax = richness.u), alpha = .2)

ggplot(data = bcm1, aes(x = month, y = Shannon)) + theme_bw() + 
  scale_x_continuous(name = "Month") + scale_y_continuous(name = "Shannon index") +
  geom_jitter(mapping = aes(colour = Mouse), width = .1) +
  geom_line(data = bcm.pred.pw, mapping = aes(x = month, y = Shannon)) +
  geom_ribbon(data = bcm.pred.pw, mapping = aes(ymin = Shannon.l, ymax = Shannon.u), alpha = .2)

ggplot(data = bcm1, aes(x = month, y = Simpson)) + theme_bw() + 
  scale_x_continuous(name = "Month") + scale_y_continuous(name = "Simpson index") +
  geom_jitter(mapping = aes(colour = Mouse), width = .1) +
  geom_line(data = bcm.pred.pw, mapping = aes(x = month, y = Simpson)) +
  geom_ribbon(data = bcm.pred.pw, mapping = aes(ymin = Simpson.l, ymax = Simpson.u), alpha = .2)

ggplot(data = bcm1, aes(x = month, y = hill.3)) + theme_bw() + 
  scale_x_continuous(name = "Month") + scale_y_continuous(name = "Hill number (3)") +
  geom_jitter(mapping = aes(colour = Mouse), width = .1) +
  geom_line(data = bcm.pred.pw, mapping = aes(x = month, y = hill.3)) +
  geom_ribbon(data = bcm.pred.pw, mapping = aes(ymin = hill.3.l, ymax = hill.3.u), alpha = .2)

ggplot(data = bcm1, aes(x = month, y = hill.4)) + theme_bw() + 
  scale_x_continuous(name = "Month") + scale_y_continuous(name = "Hill number (4)") +
  geom_jitter(mapping = aes(colour = Mouse), width = .1) +
  geom_line(data = bcm.pred.pw, mapping = aes(x = month, y = hill.4)) +
  geom_ribbon(data = bcm.pred.pw, mapping = aes(ymin = hill.4.l, ymax = hill.4.u), alpha = .2)

ggplot(data = bcm1, aes(x = month, y = hill.5)) + theme_bw() + 
  scale_x_continuous(name = "Month") + scale_y_continuous(name = "Hill number (5)") +
  geom_jitter(mapping = aes(colour = Mouse), width = .1) +
  geom_line(data = bcm.pred.pw, mapping = aes(x = month, y = hill.5)) +
  geom_ribbon(data = bcm.pred.pw, mapping = aes(ymin = hill.5.l, ymax = hill.5.u), alpha = .2)

ggplot(data = bcm1, aes(x = month, y = hill.6)) + theme_bw() + 
  scale_x_continuous(name = "Month") + scale_y_continuous(name = "Hill number (6)") +
  geom_jitter(mapping = aes(colour = Mouse), width = .1) +
  geom_line(data = bcm.pred.pw, mapping = aes(x = month, y = hill.6)) +
  geom_ribbon(data = bcm.pred.pw, mapping = aes(ymin = hill.6.l, ymax = hill.6.u), alpha = .2)
```
...
#plot
ggplot(abund, aes(x=estimate,y=est)) + geom_boxplot(aes(group=estimate)) + theme_bw()+ ylab("diversity")+ xlab("")
...
#ggplot(d, aes(x=organ, y=chao, color=type)) + geom_boxplot(aes(group=interaction(organ,type))) + ylab("diversity (chao2)")+ xlab("")+ labs(title="myeloid cells across organ at month 15")
#plot
d <- d[d$type!="SP",]
ggplot(d, aes(x=type, y=chao, color=organ)) + geom_boxplot(aes(group=interaction(organ,type))) + ylab("diversity (chao2)")+ xlab("") + theme_bw()
#+ labs(title="myeloid cells across organ at month 15")
...
z3 <- z3[z3$mice!="0",]
#plot S2B
ggplot(z3, aes (x= Pgen, fill=mice)) +geom_histogram(alpha=0.8)+  scale_x_log10() + ylab("density") + xlab("Pgen")+ theme_bw()+ labs(fill='')+ scale_fill_discrete(labels=c( "unique barcode", "barcode shared by 2/4 mice", "barcode shared by 3/4 mice", "barcode shared by 4/4 mice"))
...
#plot S2E
ggplot(WW, aes(x=factor(threshold), y=shared)) + geom_boxplot() + ylab("fraction barcode shared between mice")+ xlab("threshold 10^-x")
...
#plot S2D
ggplot(WW, aes(x=factor(threshold), y=deleted, color=factor(mouse))) + geom_line(aes(group = mouse)) + geom_point()  +  labs(x="threshold 10^-x", y="fraction barcode deleted", col="mouse")
```
...
ggplot(xx[,], aes(x=factor(cut), y=freq)) + geom_boxplot() + scale_y_continuous(name="frequency(log10)", trans='log10') + xlab("Pgen(log10)")
```
...
#plots
par(mfrow=c(1,3))
hist(m, main="sharing between mice", freq=FALSE, xlim=c(0,0.5), breaks=seq(0,1,0.1),xlab="barcode shared (%)", ylab="")
...
data <- read.delim("BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 repsum.txt")
names(data)[1] <- "tag"
data <- data[,-1]
