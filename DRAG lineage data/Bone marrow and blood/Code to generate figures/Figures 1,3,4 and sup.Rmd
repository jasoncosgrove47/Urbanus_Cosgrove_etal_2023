...
#load data
D <- read.delim("inputs/BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 renorm norm cell numb repsum not blood samples-abfilt probability yuval super mouse.txt")
...
heatmap.2(m, scale = "none", col =mycol, 
...
#select data of the right Pgen threshold
T=10^-4 #define threshold
...
ggplot(p, aes(x=class, y=mean)) +geom_bar(stat="identity",color = "black", fill = c("red","darkolivegreen3","grey","plum","orange","cornflowerblue"),lwd = 2)+ labs(title = "") + ylab("barcode per class (%)") + geom_errorbar(aes(ymax=mean+(sd/2),  ymin=mean - (sd/2),width = 0.5),size = 0.5,color = "grey50")+ theme_bw()+ scale_x_discrete(drop=FALSE)+ theme_bw()+xlab("") +theme(axis.text=element_text(size=13,face="bold"),
        axis.title=element_text(size=25,face="bold")) + geom_jitter()
...
# Fig3D
ggplot(pp, aes(fill=class, x=type, y=contrib)) + geom_bar(position="fill", stat="identity",color = "black",lwd = 1.3, show.legend = FALSE) + theme_bw() + ylab("Proportional contribution to cell type") + xlab("") +scale_fill_manual(values= c("red","orange","plum","grey","darkolivegreen3","cornflowerblue"))+theme(axis.text=element_text(size=17,face="bold"),axis.title=element_text(size=20,face="bold")) 
...
MM <- M[which(M$contrib>0),]
ggplot(MM, aes (x=pop,y=contrib, color=class)) + geom_boxplot(outlier.shape = NA) + xlab("") + theme_bw() + scale_y_continuous(name = "relative cell output per barcode") + theme(axis.text=element_text(size=15,face="bold"),axis.title=element_text(size=16,face="bold"))
...
```{r, echo=FALSE}
#load data
D <- read.delim("inputs/%cell in GFP+ and neg over time.txt")
#format data
...
D<- read.delim("inputs/BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 renorm norm cell numb repsum.txt")
...
ggplot(z, aes (x=factor(month),y=var+1)) +geom_boxplot(outlier.shape = NA) + geom_jitter(aes(colour = mouse),size=3.0) + xlab("myeloid cells in bone marrow") + theme_bw() + scale_y_continuous(name = "Cell output per barcode",trans='log10')+theme(axis.text=element_text(size=15,face="bold"),axis.title=element_text(size=16,face="bold")) + scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9","red"))
dev.off()
...
ggplot(z, aes (x=factor(month),y=var+1)) +geom_boxplot(outlier.shape = NA) + geom_jitter(aes(colour = mouse), size=2, ) +xlab("Month post-induction") + theme_bw() + scale_y_continuous(name = "Cell output per barcode",trans='log10')+theme(axis.text=element_text(size=15,face="bold"),axis.title=element_text(size=16,face="bold")) + scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9","red"))
dev.off()
...
bcm <- read.table("inputs/BCM run 3 and 4 freq10frac0_allsamples_norm 1_0.003 renorm norm cell numb sumrep long format.txt",header = TRUE) %>% as_tibble
...
#load data 
d<- read.delim("inputs/BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 renorm.txt")
...
```{r, echo=FALSE}
bcm1 <- read.table("inputs/BCM run 3 and 4 freq10frac0_allsamples_norm 1_0.003 renorm hill number per rep.txt",header = TRUE)
...
#load data
dab <- read.delim("inputs/BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 renorm norm cell numb a vs b.txt")
...
#load data
D <- read.delim("inputs/BCM run 3 and 4 freq10frac0_allsamples_0.003 norm cell numb.txt")
...
```{r, echo=FALSE}
#load data
chaoall <- read.delim("inputs/BCM run 3 and 4 freq10frac0_allsamples_0.003 renorm chao table on cell numb.txt")
chaoall <- chaoall[,-1]
#selec data
...
write.table(d, "BCM run 3 and 4 freq10frac0_allsamples_0.003 renorm chao table on cell numb for prism.txt",row.names=TRUE,col.names=NA, sep="\t")
```
...
par(mfrow=c(2,2))
#load data
d <- read.delim("inputs/BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 renorm repsum complexity probability yuval.txt",sep="\t")
...
#load data
D <- read.delim("inputs/BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 renorm norm cell numb.txt")
...
```{r, echo=FALSE}
#load data
D <- read.delim("inputs/BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 renorm repsum complexity probability yuval.txt")
...
d <- read.delim("inputs/splitbc_run3and4_allsamples.txt")
...
complex <- read.delim("inputs/splitbc_run3and4_complexity.txt", stringsAsFactors=FALSE)
#load barcoding data per sample
data <- read.delim("inputs/BCM run 3 and 4 freq10frac0_allsamples norm 1_0.003 renorm norm cell numb repsum.txt")
names(data)[1] <- "tag"
data <- data[,-1]
