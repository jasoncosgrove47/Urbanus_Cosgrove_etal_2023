...
geneSetAnnotation <- function(lsks, gene.set.names){
  gene.sets <- read.csv("Fig2_inputs/genesets.csv")
...
generateSeuratObject <- function(){
  ...
  gene_bc_matrix.neg <- Read10X(data.dir="Fig2_inputs/GFPneg") 
  gene_bc_matrix.pos <-Read10X(data.dir= "Fig2_inputs/GFPpos") 
